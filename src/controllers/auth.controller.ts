    ...
    jwt.verify(
      ...
      (err, decoded) => {
        if (err || !decoded) {
          res
            .status(403)
            .json(ApiResponse.fail("Invalid or expired refresh token"));
          return;
        }
        ...
        res
          .status(200)
          .json(ApiResponse.success(null, "Refresh token successfully"));
      }
    );
  } catch (err) {
    next(err);
...
export const loginWithGoogleController = async (
  ...
  try {
    ...
    const { accessToken, refreshToken, role_name } =
