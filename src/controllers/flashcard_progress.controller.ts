...
export const answerFlashcardSessionController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.user?._id) {
            return res
                .status(401)
        ...
        res.status(200).json(
            ApiResponse.success(result, "Flashcard answer processed successfully")
        );
    } catch (err) {
        next(err);
    }
