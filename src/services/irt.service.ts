import { DayStudy, Group, LearningPath, Lesson, Quiz, TopicVocabulary, User, UserTest, WeekStudy } from "../models";
...
export const generateIRTWeeklyPlanService = async (
    ...
    const minutesPerDay = (userProfile?.hours_per_day || 0) * 60;

    const totalWeekMinutes = minutesPerDay * (userProfile?.study_days_per_week || 0);

    const weakMinutes = Math.round(totalWeekMinutes * 0.65);
    const mediumMinutes = Math.round(totalWeekMinutes * 0.25);
    const strongMinutes = Math.round(totalWeekMinutes * 0.10);
    ...
    const weekStudyDoc = await WeekStudy.create({
        no: nextWeekNo,
        description: weekly_data?.debug_log || `Week ${nextWeekNo} auto-generated by IRT`,
        ...
        for (let dIdx = 0; dIdx < totalDays; dIdx++) {
            ...
            const sessions = Array.isArray(day.sessions)
                ? day.sessions.map((session: any, sessionIdx: number) => {
                    ...
                    const items = Array.isArray(session.items) && session.items.length > 0
                        ? session.items.map((it: any, itemIdx: number) => {
                            ...
                            return {
                                kind: mappedKind,
                                activity_id: it.resource_id ? new Types.ObjectId(it.resource_id) : undefined,
                                status: isFirstItem ? WeekStudyStatus.IN_PROGRESS : WeekStudyStatus.LOCK,
                            };
                        })
                        : [];
                    ...
                    return {
                        ...
                        status: isFirstSession ? WeekStudyStatus.IN_PROGRESS : WeekStudyStatus.LOCK,
                        part_type: partNum,
                        items,
                    };
                })
    ...
    await updatedThetaInUserTestService(abilities.thetaByPart, abilities.thetaOverall, result.userTestId.toString());
