...
export const getRecentUserTestsService = async (
    ...
    const recentTests = await UserTest.aggregate<IUserRecentTest>([
        // L?c bài làm c?a user
        { $match: { user_id: userObjectId } },

        // Sort bài làm g?n nh?t
        { $sort: { submit_at: -1 } },
        { $limit: limit },

        // Join Test d? l?y thông tin d? thi
        {
            $lookup: {
                from: "tests",
                localField: "test_id",
...
export const getUserTestHistoryService = async (
    ...
    const safeLimit = Math.min(100, Math.max(1, Number(limit) || 10)); // ch?n limit quá l?n
...
export const getDemoTestTagAccuracyService = async (
  ...
  const demoTest = await UserTest.findOne({
    ...
    .populate({
      ...
      select: "tags", // Question có tru?ng tags: string[]
    })
    .lean();
