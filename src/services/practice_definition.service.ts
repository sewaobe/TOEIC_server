...
export const getAllPracticeDefinitionTopicsService = async (
    ...
    const [topics, total] = await Promise.all([
        PracticeTopicVocabulary.find(query)
            .select("title description level tags vocabulary_words isPublic created_by created_at")
            .skip(skip)
            .limit(limit)
            .sort({ created_at: -1 })
            .lean(),
    ...
    const topicsWithCount = topics.map(topic => ({
        ...topic,
        vocabulary_count: Array.isArray(topic.vocabulary_words) ? topic.vocabulary_words.length : 0
    }));
 ...
 */
export const getPracticeDefinitionTopicByIdService = async (topicId: string) => {
