...
const partNumberToLabel = (val: any) => {
  if (!val) return null;
...
const partLabelToNumber = (val: any) => {
...
const mapTopic = async (topic_vocabulary: any) => {
  ...
  return {
    ...
    part_type: partNumberToLabel(topic_vocabulary.part_type),
  ...
  const mappedTopics = await Promise.all(
    topicVocabularies.map(async (topicVocabulary) => {
      ...
      return {
        ...
        part_type: partNumberToLabel(topicVocabulary.part_type),
...
export const getAllTopicsService = async (
  ...
  // Always map topics to API-friendly shape (includes part_type label)
...
export const updateTopicService = async (
  ...
  const topicVocabulary = await TopicVocabulary.findOne({
    _id: id,
    created_by: userId,
  });
  if (!topicVocabulary) {
