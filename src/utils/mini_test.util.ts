...
async function computeGroupStats(group: any): Promise<{
    ...
    const sumB = qs.reduce(
        (sum, q: any) => sum + (q.irt_difficulty ?? 0),
        0
    );
...
async function selectGroupsForPartExactQuestions(
    ...
    const rawGroups = await Group.find({ part })
        .select("_id part questions audioUrl imagesUrl transcriptEnglish transcriptTranslation")
        .lean();
    ...
    for (const g of rawGroups) {
        ...
        if (questionCount === 0 || questionCount > targetQuestions) continue;
    ...
    //    - Nếu không có exact, lấy q gần nhất < target
    let bestQ = -1;
    if (dp[targetQuestions] !== -1) {
        bestQ = targetQuestions;
    } else {
        // fallback: lấy tổng lớn nhất <= targetQuestions
    ...
    const chosenGroupIds = chosenIndices.map(
        (i) => groupInfos[i].groupId
    );
...
export async function generateNextWeekMiniTest(
    ...
    for (let part = 1; part <= 7; part++) {
        ...
        cleanTheta[part] =
            typeof theta === "number" && !isNaN(theta) ? theta : 0;
    }
    ...
    for (let part = 1; part <= 7; part++) {
        ...
        const { groupIds, totalQuestions } =
            await selectGroupsForPartExactQuestions(
                part,
                thetaPart,
                targetQ
            );
        ...
        console.log(
            `Part ${part} → targetQ=${targetQ}, selectedGroups=${groupIds.length}, totalQuestions=${totalQuestions}`
        );
    ...
    const clonedGroupIds = clonedGroups.map(
        (g) => g._id as Types.ObjectId
    );
