...
const USER_ID = "6a4b3f4b8d2c609aa9b196c4";
...
async function main() {
  ...
  if (keepLearningPathIds.length > 0) {
    const currentIdQuery =
      typeof learningPathQuery._id === "object" &&
      learningPathQuery._id !== null
  ...
  const dayIdsFromWeekStudies = weekStudies.flatMap(
    (week: any) => week.days ?? [],
  );
  ...
  const dayStudies =
    dayStudyOrConditions.length > 0
      ? await DayStudy.find({ $or: dayStudyOrConditions }).select("_id").lean()
      : [];
  ...
  const groupUserIds = uniqObjectIds(
    groupUsersByLearningPath.map((g) => g._id),
  );
